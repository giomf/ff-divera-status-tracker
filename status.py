...
_HEADER=['Total on duty', 'Weekend on duty', 'Off duty with note']
...
def calculate_on_duty_percentage(input: Path, off_duty_keyword: str, output: Path):
    ...
    for name in names:
        ...
        with_note_percentage = _calculate_off_duty_with_note_percentage(name_data, off_duty_keyword)
        result[name] = [total_percentage, weekend_percentage, with_note_percentage]
...
def _calculate_on_duty_total_percentage(data: pd.Series, off_duty_keyword: str) -> float:
    ...
    return _calculate_reverse_percentage(total_off_duty_count, total_count)
...
def _calculate_on_duty_weekend_percentage(data: pd.Series, off_duty_keyword: str) -> float:
    ...
    return _calculate_reverse_percentage(weekend_off_duty_count, weekend_total_count)

def _calculate_off_duty_with_note_percentage(data: pd.DataFrame, off_duty_keyword: str) -> float:
    total_off_duty_count = data['Status'].str.count(off_duty_keyword).sum()   
    off_duty_with_note = len(data[(data['Status'] == off_duty_keyword) & (data['Note'].notnull())])    
    return _calculate_percentage(off_duty_with_note, total_off_duty_count)

def _calculate_reverse_percentage(part: int, total: int) -> float:
